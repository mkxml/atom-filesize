...
function addPrettySize(info, { useKibibyteRepresentation = true }) {
  ...
  const prettySize = getPrettySize(size, base, suffixes);
...
function addPrettyDateInfo(info, { use24HourFormat = true }) {
  ...
  const prettyDateCreated = moment(info.dateCreated).format(`MMMM Do YYYY, ${hourFormat}`);
  const prettyDateChanged = moment(info.dateChanged).format(`MMMM Do YYYY, ${hourFormat}`);
  return Object.assign(info, { prettyDateCreated, prettyDateChanged });
}

function addGzipSize(info, { useKibibyteRepresentation = true }) {
  gzipSize = gzipSize || require('gzip-size');
  const size = gzipSize.sync(fs.readFileSync(info.absolutePath));
  const base = (useKibibyteRepresentation) ? KIBIBYTE_BASE : SI_BASE;
  const suffixes = (useKibibyteRepresentation) ? KIBIBYTE_REPRESENTATION : SI_REPRESENTATION;
  const prettySize = getPrettySize(size, base, suffixes);
  return Object.assign(info, { gzipSize: prettySize });
}
