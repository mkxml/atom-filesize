...
function createTooltip(target, htmlContent) {
  return atom.tooltips.add(target, {
    template: `<div class="tooltip" role="tooltip">
               ...
               <div class="tooltip-inner" style="min-width: 36em; padding: .5em;"></div></div></div>`,
    title: htmlContent,
    placement: 'top',
    trigger: 'manual',
    delay: 0,
...
function getHTMLForTooltip(info) {
  ...
  infoContainer.style.cssText = `display: block; border: 0;
                                 ...
                                 width: 100%; background: black; color: #DDD; opacity: .75;
  ...
  function prepareTooltip(element, info, currentConfig) {
    return (() => {
      if (!tooltip) {
        ...
        if (currentConfig.showGzippedSize) content = addGzipSize(info, content);
